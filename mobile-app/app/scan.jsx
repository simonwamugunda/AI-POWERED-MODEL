...
export default function Scan() {
  ...
  async function analyze() { setLoading(true); try { const prediction = await predictImage(asset); router.push({ pathname:'/result', params:{ scan: JSON.stringify({ ...prediction, imageUri: asset.uri }) } }); } catch(e) { Alert.alert('Prediction unavailable', e.message); } finally { setLoading(false); } }
